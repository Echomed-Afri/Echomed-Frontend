...
function ProtectedRoute({
  ...
  // Always show loading state while initialization is in progress
  if (state.isLoading) {
    return (
  ...
  if (!requireAuth && state.isAuthenticated) {
    ...
    return (
      <Navigate
        to={userType === "doctor" ? "/doctor-dashboard" : "/home"}
        replace
      />
    );
  }
...
function AppContent() {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      const userName = localStorage.getItem("echomed_user_name");
      const userEmail = localStorage.getItem("echomed_user_email");
      ...
      if (token && userType) {
        ...
        dispatch({ type: "SET_USER", payload: basicUser as any });
        dispatch({ type: "SET_AUTHENTICATED", payload: true });
        dispatch({
          type: "SET_USER_TYPE",
          payload: userType as "patient" | "doctor",
        });
        ...
        try {
          ...
          // For patients, we could add similar logic here if needed
        } catch (error) {
          console.error("Failed to load full user data:", error);
          // Keep basic user data even if API fails
        }
